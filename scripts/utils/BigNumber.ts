...
export default class BigNumber {
    private static aLetters: any = null;//["K", 3, "M", 6, "B", 9, "T", 12, "aa", 15, "bb", 18, "cc", 21,"dd", 24, "ee", 27, "ff", 30, "gg", 33, "hh", 36, "ii", 39,"jj",42,'kk',45,'ll',48,'mm',51,'nn',54,'oo',57,'pp',60,'qq',63,'rr',66,'ss',69,'tt',72,'uu',75,'vv',78,'ww',81,'xx',84,'yy',87,'zz',90,'aaa',93,'bbb',96,'ccc',99,'ddd',102,'eee',105,'fff',108,'ggg',111,'hhh',114,'iii',117,'jjj',120,'kkk',123,'lll',126,'mmm',129,'nnn',132,'ooo',135,'ppp',138,'qqq',141,'rrr',144,'sss',147,'ttt',150,'uuu',153,'vvv',156,'www',159,'xxx',162,'yyy',165,'zzz'];

    private static genLetters()
    {
        if(!this.aLetters)
        {
            let tmp2 = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z"];
            let out = ["K", 3, "M", 6, "B", 9, "T", 12];
            let index = 0
            for (var i = 15; i < 400; i += 3) {;
                let cn = Math.floor(index / 26) + 2;
               // if (cn < 2) cn = 2;
                let key = "";
                for (var j = 0; j < cn; ++j) {
                key += tmp2[(index % 26)];
